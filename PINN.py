...
def get_config() -> Dict[str, Any]:
    return {
        ...
        # ---------------- 模型名称/输出 ----------------
        "model_name": "MLP Raw Input (2D Poisson)",
        "model_filename": "poisson_mlp_raw_input.pt",
        "plot_filename_prefix": "poisson_mlp_raw_input_MLP",
...
class PoissonMLP(nn.Module):
    """用于二维 Poisson PINN 的纯 MLP（原始坐标输入版本）。"""
...
# ============================================================
def run_helmholtz():
    print("========== 当前运行 [纯 MLP + 原始坐标输入版：2D Poisson] ==========\n")
    device = get_device()
